...
public class Buyer {
	...
	private final String buyerStrategy;
	...
	public double getBuyersSurpleus() {
	...
	public void setBuyerSurplus(int k) {
		...
		double surplus = value(k) - paidPrice;
	...
	public static double futureExpectedSurplus(double gamma, int k, int actualArrivalTime, int reportedArrivalTime, double priceIfTransition, double priceIfNoTransition, double transitionProbability) {

		double expectedSurplus = gamma*transitionProbability*surplus(gamma, k+1, actualArrivalTime, reportedArrivalTime+1, priceIfTransition);
		expectedSurplus += gamma*(1-transitionProbability)*surplus(gamma, k, actualArrivalTime, reportedArrivalTime+1, priceIfNoTransition);
		return expectedSurplus;
	}
